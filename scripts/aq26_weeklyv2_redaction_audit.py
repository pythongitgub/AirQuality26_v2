...
PATTERNS=[r"(?i)apiKey=[A-Za-z0-9_\-\.]{8,}",r"(?i)apikey=[A-Za-z0-9_\-\.]{8,}",r"(?i)api_key=[A-Za-z0-9_\-\.]{8,}",r"(?i)token=[A-Za-z0-9_\-\.]{8,}",r"(?i)password=[^&\s\"']{6,}",r"(?i)client_secret=[^&\s\"']{6,}",r"(?i)Bearer\s+[A-Za-z0-9_\-\.]{12,}"]
...
def main():
    ...
    for p in root.rglob('*'):
        ...
        scanned+=1; txt=p.read_text(encoding='utf-8', errors='ignore')
        for pat in PATTERNS:
            m=re.search(pat, txt)

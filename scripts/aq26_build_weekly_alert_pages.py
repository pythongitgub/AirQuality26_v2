...
Builds public redacted and protected unredacted weekly update pages, injects a
front-page weekly alert, copies logo/favicon/banner assets safely, and writes
machine-readable weekly alert JSON summaries.

Public output is legally cautious:
...
UNREDACTED_NOTICE = (
    "Internal review area. Contains diagnostics, candidate scores, backfill status "
    "and workflow provenance for QA/review. Do not publish raw diagnostics externally."
)
STATUS_LABELS = {
...
def safe_copy(src: Path, dst: Path) -> bool:
    """Copy src to dst unless they are the same file. Returns True if copied."""
    try:
...
def summarise(rows: List[Dict[str, str]], overlay_summary: Dict[str, Any], backfill_status: Dict[str, Any]) -> Dict[str, Any]:
    ...
    status_txt = str(backfill_status.get("status", "")).lower()
    backfill_ok = status_txt in {"success", "ok", "completed", "harvested", "attempted", "not_run", "not_run_in_this_workflow", "completed_with_script_warnings"}
    level = "red" if not backfill_ok else "amber" if unresolved > 0 or candidates > 0 else "green"
    headline = "Weekly update needs review" if level == "red" else "Weekly incinerator evidence update" if level == "amber" else "Weekly update complete"
...
.aq26-alert__bar{display:flex;gap:1rem;justify-content:space-between;align-items:center;padding:.85rem 1rem;background:linear-gradient(90deg,#071a30,#0b4d83,#006c67);color:#fff}.aq26-alert__bar b{letter-spacing:.06em;text-transform:uppercase}.aq26-alert__bar span{opacity:.92}
.aq26-alert__body{display:grid;grid-template-columns:1.6fr 2fr;gap:1rem;padding:1.2rem}.aq26-alert__body h2{font-size:1.65rem;margin:.1rem 0 .5rem;color:#081d34}.aq26-alert__body p{color:#43556a;line-height:1.55}
.aq26-alert__stats{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:.75rem}.aq26-alert__stat{border:1px solid #e3edf7;border-radius:16px;padding:.85rem;background:#f8fbff}.aq26-alert__stat .k{font-size:.72rem;text-transform:uppercase;letter-spacing:.12em;font-weight:900;color:#66778d}.aq26-alert__stat .v{font-size:1.75rem;font-weight:950;color:#0b2540;margin:.2rem 0}
...
<section class="aq26-video-banner" aria-label="AQ26 moving evidence banner">
  <video data-aq26-banner-video autoplay muted loop playsinline preload="metadata" poster="assets/air_quality_web.svg">
...
def alert_html(summary: Dict[str, Any], unredacted: bool = False) -> str:
    ...
    actions = "<a class='aq26-alert__btn' href='weekly-update.html'>Open weekly update</a><a class='aq26-alert__btn alt' href='overlays.html'>Overlay status</a>"
    if unredacted:
        actions = "<a class='aq26-alert__btn' href='weekly-update.html'>Open unredacted weekly update</a><a class='aq26-alert__btn alt' href='diagnostics.html'>Diagnostics</a><a class='aq26-alert__btn alt' href='candidates.html'>Candidates</a>"
    return f"""
...
<section class="aq26-alert {esc(level)}" role="status" aria-live="polite">
  ...
  <div class="aq26-alert__body"><div><h2>{esc(summary.get('headline'))}</h2><p>{esc(notice)}</p><div class="aq26-alert__actions">{actions}</div></div>
...
def basic_page(title: str, body: str, unredacted: bool = False) -> str:
    ...
    nav = "<a href='index.html'>Home</a> <a href='weekly-update.html'>Weekly update</a> <a href='incinerators.html'>Incinerators</a> <a href='overlays.html'>Overlays</a>"
    if unredacted:
...
def build_weekly_page(summary: Dict[str, Any], rows: List[Dict[str, str]], unredacted: bool = False) -> str:
    body = video_banner(summary) + ticker(summary) + alert_html(summary, unredacted=unredacted)
    unresolved = ", ".join(summary.get("unresolved_names") or []) or "None"
    body += f"""
<section class="card section"><h1>Weekly AQ26 incinerator evidence update</h1><p>This page is regenerated by the Monday 07:00 UK workflow. It summarises the facility overlay state, focused backfill status and review queue.</p><div class="legal"><b>Safety notice:</b> {esc(UNREDACTED_NOTICE if unredacted else PUBLIC_NOTICE)}</div></section>
<section class="grid stats"><div class="card stat"><div class="label">Facilities</div><div class="value">{esc(summary['total_facilities'])}</div><div class="note">England/Wales register spine</div></div><div class="card stat"><div class="label">Validated</div><div class="value">{esc(summary['validated_overlays'])}</div><div class="note">Retained validated overlays</div></div><div class="card stat"><div class="label">Under review</div><div class="value">{esc(summary['candidate_overlays'])}</div><div class="note">Candidate monitoring overlays</div></div><div class="card stat"><div class="label">Fallback</div><div class="value">{esc(summary['unresolved_facilities'])}</div><div class="note">{esc(unresolved)}</div></div></section>
<section class="card section"><h2>AI-assisted triage summary</h2><p>The current overlay-path coverage is <b>{esc(summary['overlay_path_coverage_pct'])}%</b>. This is a transparent triage signal based on validated overlays, selected review candidates and unresolved fallback cases. It is not a regulatory or causal conclusion.</p></section>
"""
...
def ensure_assets(site: Path, repo: Path) -> None:
    ...
    for name in ["air_quality_web.svg", "favicon.svg", "logo_web.svg"]:
        for src in [repo / "website/assets" / name, repo / "site_public/assets" / name, repo / "site_unredacted/assets" / name]:
            if safe_copy(src, assets / name):
                if name == "favicon.svg":
                    safe_copy(src, site / "favicon.svg")
                break
            elif src.exists() and (assets / name).exists():
                if name == "favicon.svg":
                    safe_copy(src, site / "favicon.svg")
                break
    for src_dir in [repo / "website/assets/banners", repo / "site_public/assets/banners", repo / "site_unredacted/assets/banners"]:
        if src_dir.exists():
            dst = assets / "banners"
            dst.mkdir(parents=True, exist_ok=True)
            for p in sorted(src_dir.glob("*.webm")):
                safe_copy(p, dst / p.name)
...
def inject_front_page(site: Path, summary: Dict[str, Any], unredacted: bool = False) -> None:
    ...
    # Also avoid stacking repeated banner/ticker blocks from previous runs.
    txt = re.sub(r'<section class="aq26-video-banner".*?</section>\s*', "", txt, flags=re.S)
    txt = re.sub(r"<div class='aq26-weekly-ticker'.*?</div></div>\s*", "", txt, flags=re.S)
...
def main() -> int:
    ...
    ap.add_argument("--backfill-status", default="", help="Optional explicit backfill status JSON path")
    args = ap.parse_args()

    repo = Path(args.repo_root).resolve()
    public = (repo / args.public_site).resolve()
    unredacted = (repo / args.unredacted_site).resolve()
    ...
    (public / "weekly-update.html").write_text(build_weekly_page(summary, rows, unredacted=False), encoding="utf-8")
    (unredacted / "weekly-update.html").write_text(build_weekly_page(summary, rows, unredacted=True), encoding="utf-8")
    (unredacted / "robots.txt").write_text("User-agent: *\nDisallow: /\n", encoding="utf-8")

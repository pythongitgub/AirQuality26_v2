...
import argparse, mimetypes, os, smtplib, ssl, sys
...
def latest_run(root: Path) -> Path:
    ...
    if not runs: raise FileNotFoundError(f"No AQ26_WEEKLY_* runs under {root}")
    return runs[-1]
...
def main() -> int:
    ...
    msg["From"] = mail_from
    msg["To"] = mail_to
    msg["Subject"] = f"AQ26 weekly validated report — {run.name}"
    msg.set_content(f"Attached is the AQ26 weekly validated report bundle.\n\nRun folder: {run}\n\nThis is an automated controlled-review evidence pack. No third-party endorsement is implied.\n")
    if attachment: attach_file(msg, attachment)
    ...
    if port == 465:
        ...
        with smtplib.SMTP(host, port) as s:
            s.starttls(context=context); s.login(username, password); s.send_message(msg)
    print(f"Email sent to {mail_to} with attachment {attachment}")
    return 0

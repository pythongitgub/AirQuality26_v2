...
def main():
    ...
    # CDSE gates: distinguish OData download from Sentinel Hub OAuth.
    gates["cdse_catalogue_ready"] = bool(cdse.get("cdse_catalogue_ready", gates.get("satellite_catalogue_ready", False)))
    gates["cdse_odata_username_password_ready"] = bool(cdse.get("odata_password_grant", {}).get("download_auth_ready"))
    gates["cdse_sentinelhub_client_credentials_ready"] = bool(cdse.get("sentinelhub_client_credentials", {}).get("sentinelhub_auth_ready"))
    gates["cdse_download_ready"] = bool(cdse.get("cdse_download_ready"))
    gates["cdse_auth_ready"] = bool(cdse.get("cdse_auth_ready"))
